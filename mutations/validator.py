...
class MutationValidation:
    ...
    def plot_confusion_matrix(self, cm, labels, output_path):
        ...
        if cm is None:
            plt.text(0.5, 0.5, 'No Confusion Matrix: Perfect prediction', horizontalalignment='center', verticalalignment='center', fontsize=20)
            plt.axis('off')
        else:
...
def run_validation_analysis(midi_name: str, results_dir: Path) -> Dict[str, float]:
    ...
    for category, metrics in category_metrics.items():
        ...
        validator.plot_confusion_matrix(metrics["confusion_matrix"], metrics["labels"], str(category_confusion_matrix_path))
