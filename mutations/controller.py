...
def pitch_shift_mutation(excerpt, min_pitch=MIN_PITCH_DEFAULT, max_pitch=MAX_PITCH_DEFAULT, distribution=None):
    ...
    log = NoteMutationDetail(change_type="pitch", onset_timestamp=mutation.loc[index, "onset"], pitch=mutation.loc[index, "pitch"])
...
def note_played_too_soon_mutation_controller(excerpt, tempo=120):
    ...
    log = NoteMutationDetail(change_type="early", onset_timestamp=mutation.loc[index, "onset"], pitch=mutation.loc[index, "pitch"])
    return mutation, get_mutation_log(mutation, log, index)
