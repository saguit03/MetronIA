...
class TempoAnalyzer:
    ...
    def analyze_tempo_with_reference(self, reference_audio: np.ndarray, live_audio: np.ndarray,
                                     sr: int, reference_tempo: Optional[float] = None) -> TempoAnalysisResult:
        if reference_tempo:
            tempo_ref = reference_tempo
        else:
            candidates_ref = self.extract_multiple_tempo_candidates(reference_audio, sr)
    ...
    def extract_multiple_tempo_candidates(self, audio: np.ndarray, sr: int, start_tempo: Optional[int] = None) -> List[
        float]:
        tempos = []
